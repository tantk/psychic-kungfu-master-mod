...
| File | Where to paste |
|---|---|
| `steam-guide-zh.bbcode` | Steam Community Guide (Chinese, primary audience) — <https://steamcommunity.com/app/3313720/guides/> |
| `steam-guide-en.bbcode` | Steam Community Guide (English, secondary) — same URL, separate guide |
...
4. Title — keep it neutral; don't use the word "cheat" or "trainer" in the title. The Chinese guide title `今古群侠传 · 民间修改器 + 数据查询站` is fine ("民间修改器" = community modifier).
...
Drag-and-drop into the editor in this order (first image = thumbnail in the guide list):

1. Mod UI window next to the game — wuxia theme visible, leader card showing 银两 + game time
2. Items tab with the 物品 category filter open + sorted-by-value
3. NPCs tab showing a recognizable character with kungfu loadout, OR the Pages site's gifts-by-NPC view

## Caveat — Steam Guide policy on cheats

Steam doesn't outright ban single-player cheat-mod guides, but **the game developer can request a guide be removed** and Steam usually complies without warning. Mitigations baked into these BBCodes:
...
When the README, release version, or feature list changes substantively, update both `.bbcode` files. They're cheap to regenerate — see the conversation history for the original drafts.
